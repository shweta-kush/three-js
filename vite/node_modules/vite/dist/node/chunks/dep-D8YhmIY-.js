...
import path$m, { win32 as win32$1, posix as posix$1, isAbsolute as isAbsolute$1, join as join$2, extname as extname$1, dirname as dirname$2, relative as relative$2, basename as basename$2 } from 'node:path';
import { fileURLToPath, URL as URL$3, parse as parse$j, pathToFileURL } from 'node:url';
...
picomatch$5.matchBase = (input, glob, options, posix = utils$l.isWindows(options)) => {
  const regex = glob instanceof RegExp ? glob : picomatch$5.makeRe(glob, options);
  return regex.test(path$k.basename(input));
};
...
const dataToEsm = function dataToEsm(data, options = {}) {
    ...
    for (const [key, value] of Object.entries(data)) {
        if (key === makeLegalIdentifier(key)) {
            ...
            defaultExportRows.push(`${stringify$d(key)}:${_}${serialize(value, options.compact ? null : t, '')}`);
...
var path$j = require$$0$4;

var commondir = function (basedir, relfiles) {
    if (relfiles) {
        var files = relfiles.map(function (r) {
            return path$j.resolve(basedir, r);
        });
    }
    else {
        var files = basedir;
...
function expand$5(str, isTop) {
  ...
  if (/\$$/.test(m.pre)) {    
    ...
    if (isSequence) {
      ...
      if (n.length === 1) {
        // x{{a,b}}y ==> x{a}y x{b}y
        n = expand$5(n[0], false).map(embrace);
        if (n.length === 1) {
...
var expand$6 = /*@__PURE__*/getDefaultExportFromCjs(braceExpansion);
...
/* c8 ignore stop */
const sep = defaultPlatform$2 === 'win32' ? path$i.win32.sep : path$i.posix.sep;
minimatch.sep = sep;
...
const fsFromOption = (fsOption) => !fsOption || fsOption === defaultFS || fsOption === fs$i ?
    defaultFS
    : {
        ...defaultFS,
        ...fsOption,
...
function invalidatePackageData(packageCache, pkgPath) {
  const pkgDir = normalizePath$3(path$m.dirname(pkgPath));
...
function resolvePackageData(pkgName, basedir, preserveSymlinks = false, packageCache) {
  if (pnp) {
    ...
    try {
      ...
      if (!pkg) return null;
      const pkgData = loadPackageData(path$m.join(pkg, "package.json"));
  ...
  while (basedir) {
    ...
    const pkg = path$m.join(basedir, "node_modules", pkgName, "package.json");
    try {
    ...
    const nextBasedir = path$m.dirname(basedir);
...
function findNearestPackageData(basedir, packageCache) {
  ...
  while (basedir) {
    ...
    const pkgPath = path$m.join(basedir, "package.json");
...
function findNearestMainPackageData(basedir, packageCache) {
  ...
  return nearestPackage && (nearestPackage.data.name ? nearestPackage : findNearestMainPackageData(
    path$m.dirname(nearestPackage.dir),
    packageCache
  ));
}
...
function watchPackageDataPlugin(packageCache) {
  ...
  packageCache.set = (id, pkg) => {
    if (!isInNodeModules$1(pkg.dir) && !watchedDirs.has(pkg.dir)) {
      ...
      watchFile(path$m.join(pkg.dir, "package.json"));
    }
  ...
  return {
    ...
    watchChange(id) {
      if (id.endsWith("/package.json")) {
        invalidatePackageData(packageCache, path$m.normalize(id));
      }
    },
    handleHotUpdate({ file }) {
      if (file.endsWith("/package.json")) {
        invalidatePackageData(packageCache, path$m.normalize(file));
      }
    }
  };
}
...
function traverseBetweenDirs(longerDir, shorterDir, cb) {
  while (longerDir !== shorterDir) {
    cb(longerDir);
    longerDir = path$m.dirname(longerDir);
  }
}
...
function resolveDependencyVersion(dep, pkgRelativePath = "../../package.json") {
  const pkgPath = path$m.resolve(_require$1.resolve(dep), pkgRelativePath);
...
function normalizePath$3(id) {
  return path$m.posix.normalize(isWindows$3 ? slash$1(id) : id);
}
...
const isJSRequest = (url) => {
  ...
  if (!path$m.extname(url) && url[url.length - 1] !== "/") {
    return true;
  }
  return false;
};
...
function prettifyUrl(url, root) {
  ...
  if (isAbsoluteFile || url.startsWith(FS_PREFIX)) {
    const file = path$m.posix.relative(
      root,
...
function lookupFile(dir, fileNames) {
  while (dir) {
    for (const fileName of fileNames) {
      const fullPath = path$m.join(dir, fileName);
      if (tryStatSync(fullPath)?.isFile()) return fullPath;
    }
    const parentDir = path$m.dirname(dir);
...
function isFilePathESM(filePath, packageCache) {
  if (/\.m[jt]s$/.test(filePath)) {
    ...
    try {
      const pkg = findNearestPackageData(path$m.dirname(filePath), packageCache);
...
function emptyDir(dir, skip) {
  ...
  for (const file of fs__default.readdirSync(dir)) {
    ...
    if (nested?.has(file)) {
      emptyDir(path$m.resolve(dir, file), nested.get(file));
    } else {
      fs__default.rmSync(path$m.resolve(dir, file), { recursive: true, force: true });
    }
  }
}
function copyDir(srcDir, destDir) {
  ...
  for (const file of fs__default.readdirSync(srcDir)) {
    const srcFile = path$m.resolve(srcDir, file);
    if (srcFile === destDir) {
      continue;
    }
    const destFile = path$m.resolve(destDir, file);
...
async function recursiveReaddir(dir) {
  ...
  const files = await Promise.all(
    dirents.map((dirent) => {
      const res = path$m.resolve(dir, dirent.name);
...
function optimizeSafeRealPathSync() {
  ...
  try {
    fs__default.realpathSync.native(path$m.resolve("./"));
  } catch (error) {
...
function ensureWatchedFile(watcher, file, root) {
  ...
  !file.includes("\0") && fs__default.existsSync(file)) {
    watcher.add(path$m.resolve(file));
  }
}
...
async function resolveServerUrls(server, options, config) {
  ...
  if (hostname.host !== void 0 && !wildcardHosts.has(hostname.host)) {
    ...
    Object.values(os$4.networkInterfaces()).flatMap((nInterface) => nInterface ?? []).filter(
...
const _dirname = path$m.dirname(fileURLToPath(import.meta.url));
...
function hasCorrectCase(file, assets) {
  ...
  const parent = path$m.dirname(file);
  if (fs__default.readdirSync(parent).includes(path$m.basename(file))) {
  ...
  const logTransform = throttle((id) => {
    writeLine(
      `transforming (${transformedCount}) ${colors$1.dim(
        path$m.relative(config.root, id)
      )}`
    );
  });
  return {
    ...
    async writeBundle({ dir: outDir }, output) {
      ...
      if (shouldLogInfo) {
        ...
        const relativeOutDir = normalizePath$3(
          path$m.relative(
            config.root,
            path$m.resolve(config.root, outDir ?? config.build.outDir)
          )
        );
        const assetsDir = path$m.join(config.build.assetsDir, "/");
...
const POSIX_SEP_RE = new RegExp('\\' + path$m.posix.sep, 'g');
const NATIVE_SEP_RE = new RegExp('\\' + path$m.sep, 'g');
...
const IS_POSIX = path$m.posix.sep === path$m.sep;
...
async function resolveTSConfigJson(filename, cache) {
	if (path$m.extname(filename) !== '.json') {
		return; // ignore files that are not json
	}
	const tsconfig = path$m.resolve(filename);
...
const posix2native = IS_POSIX
	? (filename) => filename
	: (filename) => filename.replace(POSIX_SEP_RE, path$m.sep);
...
const native2posix = IS_POSIX
	? (filename) => filename
	: (filename) => filename.replace(NATIVE_SEP_RE, path$m.posix.sep);
...
const resolve2posix = IS_POSIX
	? (dir, filename) => (dir ? path$m.resolve(dir, filename) : path$m.resolve(filename))
	: (dir, filename) =>
			native2posix(
				dir
					? path$m.resolve(posix2native(dir), posix2native(filename))
					: path$m.resolve(posix2native(filename))
			);
...
function resolveReferencedTSConfigFiles(result, options) {
	const dir = path$m.dirname(result.tsconfigFile);
	return result.tsconfig.references.map((ref) => {
		const refPath = ref.path.endsWith('.json')
			? ref.path
			: path$m.join(ref.path, options?.configName ?? 'tsconfig.json');
...
function isIncluded(filename, result) {
	const dir = native2posix(path$m.dirname(result.tsconfigFile));
...
async function find(filename, options) {
	let dir = path$m.dirname(path$m.resolve(filename));
	...
	if (options?.root && !path$m.isAbsolute(options.root)) {
		options.root = path$m.resolve(options.root);
	}
...
function findUp(dir, { resolve, reject, promise }, options) {
	...
	const tsconfig = path$m.join(dir, options?.configName ?? 'tsconfig.json');
	fs__default.stat(tsconfig, (err, stats) => {
		if (stats && (stats.isFile() || stats.isFIFO())) {
			...
			let parent;
			if (root === dir || (parent = path$m.dirname(dir)) === dir) {
				resolve(null);
			} else {
...
async function parseFile$1(tsconfigFile, cache, skipCache) {
	...
	const promise = promises$1
		...
		.then((json) => {
			...
			return {
				tsconfigFile,
				tsconfig: normalizeTSConfig(parsed, path$m.dirname(tsconfigFile))
			};
		})
		.catch((e) => {
...
function resolveExtends(extended, from) {
	...
	if (extended[0] !== '.' && !path$m.isAbsolute(extended)) {
		try {
...
function extendTSConfig(extending, extended) {
	...
	const relativePath = native2posix(
		path$m.relative(path$m.dirname(extending.tsconfigFile), path$m.dirname(extended.tsconfigFile))
	);
...
function rebasePath(value, prependPath) {
	if (path$m.isAbsolute(value)) {
		return value;
	} else {
		// relative paths use posix syntax in tsconfig
		return path$m.posix.normalize(path$m.posix.join(prependPath, value));
	}
}
...
function isJSConfig(configFileName) {
	return path$m.basename(configFileName) === 'jsconfig.json';
}
...
async function transformWithEsbuild(code, filename, options, inMap) {
  ...
  if (!loader) {
    const ext = path$m.extname(validExtensionRE.test(filename) ? filename : cleanUrl(filename)).slice(1);
    if (ext === "cjs" || ext === "mjs") {
      loader = "js";
...
async function reloadOnTsconfigChange(changedFile) {
  if (!server) return;
  if (path$m.basename(changedFile) === "tsconfig.json" || changedFile.endsWith(".json") && tsconfckCache?.hasParseResult(changedFile)) {
...
function checkPublicFile(url, config) {
  ...
  if (publicFiles) {
    return publicFiles.has(fileName) ? normalizePath$3(path$m.join(publicDir, fileName)) : void 0;
  }
  const publicFile = normalizePath$3(path$m.join(publicDir, fileName));
...
function assetPlugin(config) {
  ...
  return {
    ...
    resolveId(id) {
      ...
      if (publicFile) {
        return config.command === "build" ? `${viteBuildPublicIdPrefix}${id}` : id;
      }
    },
    async load(id) {
...
function fileToDevUrl(id, config) {
  ...
  if (checkPublicFile(id, config)) {
    ...
    rtn = "/" + path$m.posix.relative(config.root, id);
  } else {
    rtn = path$m.posix.join(FS_PREFIX, id);
  }
...
async function fileToBuiltUrl(id, config, pluginContext, skipPublicCheck = false, forceInline) {
  ...
  if (shouldInline(config, file, id, content, pluginContext, forceInline)) {
    ...
    const referenceId = pluginContext.emitFile({
      ...
      name: path$m.basename(file),
      type: "asset",
      source: content
    });
    const originalName = normalizePath$3(path$m.relative(config.root, file));
...
async function urlToBuiltUrl(url, importer, config, pluginContext, forceInline) {
  ...
  const file = url[0] === "/" ? path$m.join(config.root, url) : path$m.join(path$m.dirname(importer), url);
...
function getChunkOriginalFileName(chunk, root, format) {
  if (chunk.facadeModuleId) {
    let name = normalizePath$3(path$m.relative(root, chunk.facadeModuleId));
    ...
    return `_` + path$m.basename(chunk.fileName);
  }
}
...
async function computeSourceRoute(map, file) {
  ...
  try {
    sourceRoot = await fsp.realpath(
      path$m.resolve(path$m.dirname(file), map.sourceRoot || "")
    );
  } catch {
  }
  return sourceRoot;
    ...
    if (sourcesContent[index] == null && sourcePath && !virtualSourceRE.test(sourcePath)) {
      sourcesContentPromises.push(
        (async () => {
          ...
          if (sourceRoot) {
            resolvedSourcePath = path$m.resolve(sourceRoot, resolvedSourcePath);
          }
...
function applySourcemapIgnoreList(map, sourcemapPath, sourcemapIgnoreList, logger) {
  ...
  for (let sourcesIndex = 0; sourcesIndex < map.sources.length; ++sourcesIndex) {
    ...
    const ignoreList = sourcemapIgnoreList(
      path$m.isAbsolute(sourcePath) ? sourcePath : path$m.resolve(path$m.dirname(sourcemapPath), sourcePath),
      sourcemapPath
    );
...
function createConvertSourceMapReadMap(originalFileName) {
  return (filename) => {
    return fsp.readFile(
      path$m.resolve(path$m.dirname(originalFileName), filename),
      "utf-8"
    );
  };
}
...
const toRegexRange$2 = (min, max, options) => {
...
const stringify$b = (start, end, options) => {
...
const toMaxLen$1 = (input, maxLength) => {
...
const toRegex$1 = (start, end, options) => {
...
const rangeError$1 = (...args) => {
  return new RangeError('Invalid range arguments: ' + util$2.inspect(...args));
};

const invalidRange$1 = (start, end, options) => {
  if (options.strictRanges === true) throw rangeError$1([start, end]);
  return [];
};

const invalidStep$1 = (step, options) => {
...
const fillNumbers$1 = (start, end, step = 1, options = {}) => {
  let a = Number(start);
  let b = Number(end);
  ...
  let toNumber = padded === false && stringify$b(start, end, options) === false;
  let format = options.transform || transform$1(toNumber);

  if (options.toRegex && step === 1) {
    return toRange$1(toMaxLen$1(start, maxLen), toMaxLen$1(end, maxLen), true, options);
  }
  ...
  if (options.toRegex === true) {
    return step > 1
      ...
      : toRegex$1(range, null, { wrap: false, ...options });
  }

  return range;
};

const fillLetters$1 = (start, end, step = 1, options = {}) => {
  if ((!isNumber$1(start) && start.length > 1) || (!isNumber$1(end) && end.length > 1)) {
    return invalidRange$1(start, end, options);
  }
  ...
  if (options.toRegex === true) {
    return toRegex$1(range, null, { wrap: false, options });
  }

  return range;
};

const fill$5 = (start, end, step, options = {}) => {
...
const compile$3 = (ast, options = {}) => {
  const walk = (node, parent = {}) => {
    ...
    if (node.nodes && node.ranges > 0) {
      ...
      const range = fill$4(...args, { ...options, wrap: false, toRegex: true, strictZeros: true });
...
const fill$3 = fillRange$1;
const stringify$a = stringify$c;
...
const expand$4 = (ast, options = {}) => {
  ...
  const walk = (node, parent = {}) => {
    ...
    if (node.invalid || node.dollar) {
      q.push(append$2(q.pop(), stringify$a(node, options)));
      return;
    }
    ...
    if (node.nodes && node.ranges > 0) {
      ...
      let range = fill$3(...args, options);
      if (range.length === 0) {
        range = stringify$a(node, options);
      }
...
const parse$e = (input, options = {}) => {
  ...
  while (index < length) {
    ...
    if (value === CHAR_ZERO_WIDTH_NOBREAK_SPACE$1 || value === CHAR_NO_BREAK_SPACE$1) {
      continue;
    }
    ...
    if (value === CHAR_DOUBLE_QUOTE$1 || value === CHAR_SINGLE_QUOTE$1 || value === CHAR_BACKTICK$1) {
      const open = value;
      let next;
    ...
    if (value === CHAR_COMMA$1 && depth > 0) {
      if (block.ranges > 0) {
        ...
        block.nodes = [open, { type: 'text', value: stringify$9(block) }];
      }
...
const stringify$8 = stringify$c;
const compile$2 = compile_1$1;
const expand$3 = expand_1$2;
const parse$d = parse_1$3;
...
const braces$3 = (input, options = {}) => {
  ...
  if (Array.isArray(input)) {
    for (const pattern of input) {
      const result = braces$3.create(pattern, options);
...
braces$3.parse = (input, options = {}) => parse$d(input, options);
...
braces$3.stringify = (input, options = {}) => {
  if (typeof input === 'string') {
    return stringify$8(braces$3.parse(input, options), options);
  }
...
micromatch$1.capture = (glob, input, options) => {
  let posix = utils$f.isWindows(options);
  let regex = picomatch$2.makeRe(String(glob), { ...options, capture: true });
  let match = regex.exec(posix ? utils$f.toPosixSlashes(input) : input);
...
function generate(input, settings) {
    ...
    const dynamicPatterns = positivePatterns.filter((pattern) => utils$e.pattern.isDynamicPattern(pattern, settings));
...
function processPatterns(input, settings) {
     ...
     */
    return patterns.map((pattern) => utils$e.pattern.removeDuplicateSlashes(pattern));
}
/**
...
function groupPatternsByBaseDirectory(patterns) {
    const group = {};
    return patterns.reduce((collection, pattern) => {
        const base = utils$e.pattern.getBaseDirectory(pattern);
    ...
    return dirents.map((dirent) => {
        ...
        if (entry.dirent.isSymbolicLink() && settings.followSymbolicLinks) {
            try {
                ...
                entry.dirent = utils$b.fs.createDirentFromStats(entry.name, stats);
            }
            catch (error) {
                if (settings.throwErrorOnBrokenSymbolicLink) {
                    throw error;
...
function readdir$2(directory, settings) {
    ...
    return names.map((name) => {
        ...
        const entry = {
            name,
            path: entryPath,
            dirent: utils$b.fs.createDirentFromStats(name, stats)
        };
        if (settings.stats) {
            entry.stats = stats;
        }
...
class Reader {
    ...
    _makeEntry(stats, pattern) {
        const entry = {
            name: pattern,
            path: pattern,
            dirent: utils$a.fs.createDirentFromStats(pattern, stats)
        };
...
class Matcher {
    ...
    _getPatternSegments(pattern) {
        const parts = utils$9.pattern.getPatternParts(pattern, this._micromatchOptions);
...
class DeepFilter {
    ...
    _filter(basePath, entry, matcher, negativeRe) {
        ...
        const filepath = utils$8.path.removeLeadingDotSegment(entry.path);
...
class EntryFilter {
    ...
    _filter(entry, positiveRe, negativeRe) {
        const filepath = utils$7.path.removeLeadingDotSegment(entry.path);
...
function stringifyPair({ key, value }, ctx, onComment, onChompKeep) {
    ...
    const valueStr = stringify$7(value, ctx, () => (valueCommentDone = true), () => (chompKeep = true));
    let ws = ' ';
...
function stringifyBlockCollection({ comment, items }, ctx, { blockItemPrefix, flowChars, itemIndent, onChompKeep, onComment }) {
    ...
    for (let i = 0; i < items.length; ++i) {
        ...
        chompKeep = false;
        let str = stringify$7(item, itemCtx, () => (comment = null), () => (chompKeep = true));
        if (comment)
...
function stringifyFlowCollection({ comment, items }, ctx, { flowChars, itemIndent, onComment }) {
    ...
    for (let i = 0; i < items.length; ++i) {
        ...
        if (comment)
            reqNewline = true;
        let str = stringify$7(item, itemCtx, () => (comment = null));
        if (i < items.length - 1)
            str += ',';
        if (comment)
            str += lineComment(str, itemIndent, commentString(comment));
